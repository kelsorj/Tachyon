...
// NOTE -- RXFCON0 and RXFCON1 are reversed in at least p18f26k80 -- possibly this is just a doc error?  doesn't seem to effect functionality?
...
void _CANEventManager(void)
{
	if (_uCANReq)
	{
		...
		{	
			...
			case 2:
				// Check for config mode
#if defined (__18F26K80) || defined(__18F25K80)
				if (CANCON & 0x80) // header doesn't bring REQOP2 out explicity
#else
...
`	// wait for Config mode
	...
	// Place the module in enhanced legacy mode (Mode 1), also maps Rx Buf 0 into access bank
	ECANCON = 0x50;
...
#endif
	{				
		// Default receive buffer handled at the end
		...
		if (RXB1CONbits.RXFUL) 
		{
			ECANCON = 0x51;
			mCANSetControlReg(1);
			_uCAN_ret = *((RXB1CON & 0x0F) + _uCANRxHndls);
		}
#endif
#if CAN_MAX_RCV_ENDP > 4
		else if (B0CONbits.RXFUL) 
		{
			ECANCON = 0x52;
			mCANSetControlReg(2);
			_uCAN_ret = *((B0CON & 0x0F) + _uCANRxHndls);
		}
#endif
#if CAN_MAX_RCV_ENDP > 6
		else if (B1CONbits.RXFUL) 
		{
			ECANCON = 0x53;
			mCANSetControlReg(3);
			_uCAN_ret = *((B1CON & 0x0F) + _uCANRxHndls);
		}
#endif
#if CAN_MAX_RCV_ENDP > 8
		else if (B2CONbits.RXFUL) 
		{
			ECANCON = 0x54;
			mCANSetControlReg(4);
			_uCAN_ret = *((B2CON & 0x0F) + _uCANRxHndls);
		}
#endif
#if CAN_MAX_RCV_ENDP > 10
		else if (B3CONbits.RXFUL) 
		{
			ECANCON = 0x55;
			mCANSetControlReg(5);
			_uCAN_ret = *((B3CON & 0x0F) + _uCANRxHndls);
		}
#endif
#if CAN_MAX_RCV_ENDP > 12
		else if (B4CONbits.RXFUL) 
		{
			ECANCON = 0x56;
			mCANSetControlReg(6);
			_uCAN_ret = *((B4CON & 0x0F) + _uCANRxHndls);
		}
#endif
#if CAN_MAX_RCV_ENDP > 14
		else if (B5CONbits.RXFUL) 
		{
			ECANCON = 0x57;
			mCANSetControlReg(7);
			_uCAN_ret = *((B5CON & 0x0F) + _uCANRxHndls);
		}
#endif
		else 
		{

...
using BioNex.Shared.LabwareCloudXmlRpcClient;
...
namespace BioNex.CustomerGUIPlugins
{
    ...
    public partial class MonsantoPhase1GUI : UserControl, ICustomerGUI, DockExternalCommunications, IHasSystemPanel, INotifyPropertyChanged
    {
        [Import("MainDispatcher")]
        Dispatcher _dispatcher { get; set; }

        private ILog _log = LogManager.GetLogger(typeof(MonsantoPhase1GUI));
        ...
        internal DeviceInterface _platemover { get; set; }
        ...
        public MonsantoPhase1GUI([Import] ExternalDataRequesterInterface data_request_interface,
                                 ...
                                 [Import("SynapsisViewModel")] SynapsisViewModel synapsis_viewmodel)
        {
            ...
            // 2
            Debug.Assert(_configuration.UpstackingRobotName != null, "You need to set the UpstackingRobotName in the config file");
            _robot = (from x in devices where x.Name == _configuration.UpstackingRobotName select x).FirstOrDefault() as RobotInterface;
            Debug.Assert(_robot != null, String.Format("Could not find a robot named '{0}' in the device database", _configuration.UpstackingRobotName));
            ...
            _platemover = (from x in devices where x.Name == _configuration.UpstackingPlateMoverName select x).FirstOrDefault() as DeviceInterface;
            Debug.Assert(_platemover != null, String.Format("Could not find a plate mover named '{0}' in the device database", _configuration.UpstackingPlateMoverName));
            // 3
            //-----------------------------------------------------------------
            ...
            try
            {
                ...
                _hive_server = new BioNex.HiveRpc.HiveServer(OnUpdatePlateFate, OnEndBatch, _configuration.JemsListenerPort);
            ...
            {
                _log.Info(String.Format("Could not configure RPC: {0}.  Please check settings in plugins\\MonsantoPhase1Config.xml", ex.Message));
            }
        ...
        public void UpstackComplete(string barcode)
        {
            ...
            catch (Exception ex)
            {
                ...
                _log.Error(string.Format("There was an error when attempting to send ReinventoryComplete message to GEMS during an Upstack operation.\nYou will need to perform a re-inventory operation for GEMS to receive the inventory information.\nThe error message was '{0}'", ex.Message));
            }
        }
        ...
        private void LoadDllConfiguration()
        {
            try
            {
                ...
                string config_path = BioNex.Shared.Utils.FileSystem.GetAppPath() + "\\config\\MonsantoPhase1Config.xml";
...
#pragma warning restore 67
        ...
        void ProcessPendingFates(string barcode, string fate)
        {
            ...
            if (_special_fates.Contains(fate))
            {
                ...
                MovePlate( (MutableString)barcode, ((DeviceInterface)_robot).Name, GetNextAvailableLocationCallback, DecodeLabwareFromBarcode(barcode), () => { OnUpdatePlateFate(barcode, fate); });
            ...
            MovePlate( (MutableString)barcode, dock, GetNextAvailableLocationCallback, DecodeLabwareFromBarcode(barcode), () => { OnUpdatePlateFate(barcode, fate); });

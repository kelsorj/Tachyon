        ...
        try
        {
            ...
            BioNex.HiveServerCallbackInterface callback =
                new BioNex.HiveServerCallbackInterface()
                {
                    ...
                    {
                        System.out.println("DestinationPlateComplete on '" + hive_name + "' destination '" + destination_barcode + "' mapping:");
                        for(TransferMap tmap : mapping)
                        {
                            //TransferMap tmap = (TransferMap)obj_map;
                            System.out.println("source '" + tmap.source_barcode 
                                + "' source well: (" + tmap.source_row + "," + tmap.source_column 
                                + ") destination well: (" + tmap.destination_row + "," + tmap.destination_column 
                                + ") transfer volume: " + tmap.transfer_volume
                                + " sensed volume: " + tmap.sensed_volume
                                );
                        }
                    }

...
{
    partial class IntegrationGui
    {
        private AutoResetEvent _reinventory_complete = new AutoResetEvent( false);
        ...
        {
            HomeAllCommand = new RelayCommand( () => ExecuteHomeAllCommand( true), CanExecuteHomeAllCommand);
            ReinventoryStaticStorageCommand = new RelayCommand( () => ExecuteReinventoryStaticStorage(false), CanExecuteReinventoryStaticStorage);
        ...
        internal void ExecuteReinventoryStaticStorage( bool blocking)
        {
            ...
            if( blocking) {
                ...
                _reinventory_complete.WaitOne();
            }
        }

        void hive_ReinventoryComplete(object sender, EventArgs e)
        {
            _reinventory_complete.Set();
        }

        internal bool CanExecuteReinventoryStaticStorage()
        {
            if( _engine == null)
                return false;

            if (_engine.IsInState(BehaviorEngine.State.NotHomed)) {
                ReinventoryStaticStorageToolTip = "Hive is not homed";
                return false;
            }
            if( Reinventorying) {
        ...
        [Import]
        internal Lazy<ICustomSynapsisQuery> _synapsisQuery;

        internal void ExecuteHomeAllCommand( bool prompt_user)
        {           
            ...
            new Thread(() =>
            {
                ExecuteHomeAllCommandHelper(prompt_user);
            }).Start();
        }

        internal bool ExecuteHomeAllCommandHelper(bool prompt_user)
        {
            ...
            if (!_synapsisQuery.Value.HomeAllDevices(show_prompt: prompt_user))
            {
        ...
        private bool CanExecutePauseCommand()
        {
            if( _engine == null)
                return false;
            // DKM 2012-05-17 added NotHomed to list because I don't want to allow the user to press Pause when the app first starts
            return !_engine.Paused && !_engine.Homing && !_engine.Reinventorying && !_engine.NotHomed;
        }

        internal void ExecuteResumeCommand()
        {
            // DKM 2011-05-27 do this to prevent potential main threadlock when pausing and doing other actions
            new Thread( () => {
                if( Thread.CurrentThread.Name == null)

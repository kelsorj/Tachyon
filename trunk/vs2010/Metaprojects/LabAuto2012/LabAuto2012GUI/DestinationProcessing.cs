...
namespace BioNex.CustomerGUIPlugins
{
    ...
    public class DestinationProcessing : IDestinationProcessing
    {
        ...
        {
            var insert_command = new SQLiteCommand( string.Format(@"INSERT OR REPLACE INTO work (source_barcode, source_row_index, source_column_index, destination_barcode, destination_row_index, destination_column_index, transfer_volume)
                                                                  VALUES ('{0}','{1}', '{2}', '{3}', '{4}', '{5}', '{6}');",
                                                                  mapping.source_barcode, mapping.source_row, mapping.source_column,
                                                                  destination_barcode, mapping.destination_row, mapping.destination_column, mapping.transfer_volume));
        ...
        public void SaveTransferMap( string destination_barcode, IList< BioNex.GemsRpc.TransferMap> mapping)
        {
            ...
            foreach( BioNex.GemsRpc.TransferMap map_item in mapping){
                sb.AppendFormat( @"INSERT OR REPLACE INTO work (source_barcode, source_row_index, source_column_index, destination_barcode, destination_row_index, destination_column_index, transfer_volume)
                                   VALUES ('{0}','{1}', '{2}', '{3}', '{4}', '{5}', '{6}');",
                                   map_item.source_barcode, map_item.source_row, map_item.source_column,
                                   destination_barcode, map_item.destination_row, map_item.destination_column, map_item.transfer_volume);
            }

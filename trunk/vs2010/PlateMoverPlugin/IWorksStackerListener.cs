...
namespace BioNex.PlateMover
{
    ...
    /// 
    /// // This code needs to be Dismembered now that we have the XML RPC connection working
    /// </remarks>
    public class IWorksStackerListener
    {
        ...
        public void StartListening( System.Net.IPAddress ip_address, int port)
        {
            ...
            _listener_thread = new Thread( ListenForConnectionRequestThread);
            _listener_thread.Name = "IWorksStacker listener";
        ...
        public void StopListening()
        {
            ...
            _listener_thread.Join(); // should join here since it's not a background thread
            _listener.Stop();
        ...
        private void ListenForConnectionRequestThread()
        {
            try {
                ...
                while( !_stop_server_event.WaitOne(500)){ //WaitHandle.WaitAny( wait_handles, 500) == WaitHandle.WaitTimeout) {
                    try {
                ...
                Debug.WriteLine( "TCP listener not running: " + ex.Message);
            }
            AlreadyListening = false;
        }
        ...
        private void RunCommandInterpreterThread( TcpClient client, CommandInterpreter interpreter)
        {
            ...
            while( WaitHandle.WaitAny( wait_handles, 500) == WaitHandle.WaitTimeout) {
                try
                {
                    ...
                    //  continue;
                    int bytes_read = stream.Read( buffer, 0, buffer_size);
                    if( bytes_read > 0) {

...
class PlanarMotorDriver:
    ...
    def async_motion(self, xbot_id: int, x: float, y: float, wait: bool = True) -> bool:
        """
        Move XBOT using async_motion_si (works around linear_motion_si signature issues).
        ...
        try:
            with self.lock:
                print(f"async_motion_si: xbot={xbot_id}, target=({x:.4f}, {y:.4f})")
                # Use async_motion_si like circular_motion_app.py does
                bot.async_motion_si(1, pm.ASYNCOPTIONS.MOVEALL, [xbot_id], [x], [y])
            
            if wait:
                return self.wait_until_idle(xbot_id)
            return True
        except Exception as e:
            import traceback
            print(f"Error in async motion: {e}")
    ...
    def linear_motion(self, xbot_id: int, x: float, y: float, 
        ...
        """
        Move XBOT in a straight line. Uses async_motion_si to avoid signature issues.
        
        Args:
            xbot_id: XBOT ID
            x: Target X position in meters
            y: Target Y position in meters
            final_speed: Final speed in m/s (ignored - async motion doesn't support this)
            max_speed: Maximum speed in m/s (ignored - async motion doesn't support this)
            max_acceleration: Maximum acceleration in m/s² (ignored - async motion doesn't support this)
        """
        # Use async_motion which works reliably
        return self.async_motion(xbot_id, x, y, wait=True)
    
    def jog(self, xbot_id: int, axis: str, distance: float, 
            max_speed: float = 0.5, max_acceleration: float = 5.0) -> bool:
        """Jog XBOT along an axis using async_motion_si."""
        if not self.connected:
        ...
        try:
            ...
            print(f"Jogging {axis.upper()}: ({current_x:.4f},{current_y:.4f}) -> ({target_x:.4f},{target_y:.4f})")
            return self.async_motion(xbot_id, target_x, target_y, wait=True)
        except Exception as e:
            import traceback

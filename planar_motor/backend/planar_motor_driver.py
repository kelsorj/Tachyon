...
# Add planar_motor/shaker to path for pmclib
planar_motor_path = os.path.join(os.path.dirname(__file__), "../shaker")
if planar_motor_path not in sys.path:
    sys.path.insert(0, planar_motor_path)
...
class PlanarMotorDriver:
    ...
    def __init__(self, pmc_ip: str = "192.168.1.100", pmc_port: Optional[int] = None):
        """
        ...
        if not PMCLIB_AVAILABLE:
            print("Warning: pmclib is not available. Driver will be in limited mode (static file serving only).")
            # Don't raise error - allow server to start for static file serving
            # Set a flag so we know pmclib isn't available
    ...
    def connect(self) -> bool:
        ...
        Follows the same pattern as python_demo.py StartingUp.run_start_up_routine()
        """
        if not PMCLIB_AVAILABLE:
        ...
        try:
            with self.lock:
                ...
                # Connect to PMC - exactly like python_demo.py
                if self.pmc_ip:
                ...
                # Gain mastership - exactly like python_demo.py
                print("Gaining mastership....")
                sys_cmd.gain_mastership()
                self.has_mastership = True
                
                # Get XBOT IDs - exactly like python_demo.py

"""
Planar Motor Controller Backend API
...
app = FastAPI(title="Planar Motor Control API")

# Mount GLTF models as static files
models_dir = os.path.join(os.path.dirname(__file__), "../../models/planar_motor")
...
async def root():
    """Root endpoint."""
    return {"message": "Planar Motor Control API", "pmc_ip": PMC_IP}
...
@app.post("/xbots/{xbot_id}/jog")
async def jog_xbot(xbot_id: int, req: JogRequest):
...
@app.post("/xbots/{xbot_id}/linear-motion")
async def linear_motion(xbot_id: int, req: LinearMotionRequest):
...
if __name__ == "__main__":
    ...
    PMC_IP = args.pmc_ip
    print(f"Starting Planar Motor API server on port {args.port}")

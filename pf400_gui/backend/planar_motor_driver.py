...
planar_motor_path = os.path.join(os.path.dirname(__file__), "../../planar_motor/shaker")
...
class PlanarMotorDriver:
    ...
    def __init__(self, pmc_ip: str = "192.168.1.100", pmc_port: Optional[int] = None):
        ...
        if not PMCLIB_AVAILABLE:
            raise RuntimeError("pmclib is not available. Please install it in planar_motor/shaker/")
    ...
    def connect(self) -> bool:
        ...
        try:
            with self.lock:
                print(f"Connecting to PMC at {self.pmc_ip}...")
                ...
                if xbot_ids_result.PmcRtn == pm.PMCRTN.ALLOK:
                    ...
                    print(f"Warning: Failed to get XBOT IDs: {xbot_ids_result.PmcRtn}")
                    self.xbot_ids = []
